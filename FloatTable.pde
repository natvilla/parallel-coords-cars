...
class FloatTable {
  ...
  float[] getColumnData(int col) {
    float[] colData = new float[rowCount];
    for (int row = 0; row < rowCount; row++) {
      if (isValid(row, col)) {
        colData[row] = getFloat(row, col);
      }  
    }
    return colData;
  }
  
  float[] getSortedColumnData(int col) {
    float[] colData = getColumnData(col);
    return sort(colData);
  }
  ...
  void swapColumnData(int col1, int col2) {
    String tempName = columnNames[col1];
    columnNames[col1] = columnNames[col2];
    columnNames[col2] = tempName;
    
    for (int i = 0; i < rowCount; i++) {
      float tempCol1 = data[i][col1];
      data[i][col1] = data[i][col2];
      data[i][col2] = tempCol1;
